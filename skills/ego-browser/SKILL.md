...
with dynamic imports such as `await import("node:fs/promises")`. This applies to
built-in modules too: a static `import { createServer } from "node:http"` produces
no output and exits 0, so a script that appears to do nothing is often a static
import of a built-in.

The embedded runtime does not inherit the calling shell's environment or working
directory:

- Custom environment variables are not passed through. Read configuration from a
  file, or substitute values into the script text before piping it in.
- `process.cwd()` is `/`, so relative paths do not resolve the way they do in a
  shell script. Resolve them from `process.env.HOME`, or use absolute paths.

Loopback networking is unavailable: `server.listen()` never fires its callback,
and `fetch("http://127.0.0.1:...")` hangs until the process exits. External HTTPS
requests work normally. Run a local server outside the runtime and call it from a
plain Node process when you need to exercise local code.
